...
import com.example.tarea23.transacciones.Transacciones;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(c.getCount()>0)
        {
            ...
            while(c.isAfterLast()==false)
            {
                ...
                Foto fotografia = new Foto(BitmapFactory.decodeByteArray(bytes, 0, bytes.length), descripcion);
                listaImagenes.add(fotografia);
                c.moveToNext();
                i++;
            }

            Adaptador myAdapter=new Adaptador(this,R.layout.view_items,listaImagenes);
